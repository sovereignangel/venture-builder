...
async function waitForVercelDeployment(
  ...
  if (!VERCEL_TOKEN) return { ready: false, error: 'No VERCEL_TOKEN' }
      ...
      if (res.ok) {
        ...
        if (deployment) {
          ...
          if (state === 'ERROR' || state === 'CANCELED') {
            return { ready: false, error: `Vercel deployment ${state.toLowerCase()}: ${deployment.url}` }
          }
  ...
  return { ready: false, error: `Deployment did not complete within ${maxWait / 1000}s` }
}
...
async function buildNew() {
  ...
  if (VERCEL_TOKEN) {
    console.log('Step 5: Waiting for Vercel deployment...')
    const deploy = await waitForVercelDeployment(repoName)

    if (!deploy.ready) {
      console.error(`Vercel deployment failed: ${deploy.error}`)
      await updateCallback({
        status: 'failed',
        errorMessage: deploy.error || 'Vercel deployment failed',
        repoUrl: repo.html_url,
        repoName,
        filesGenerated: files.length,
      })
      await sendTelegram([
        `*${spec.name} deployment failed*`,
        '',
        `_${deploy.error}_`,
        '',
        `GitHub: ${repo.html_url}`,
        `The repo is pushed — you can retry from Vercel or the dashboard.`,
      ].join('\n'))
      throw new Error(deploy.error || 'Vercel deployment failed')
    }

    if (deploy.url) previewUrl = deploy.url
  }

  // Step 6: Success callback
  console.log('Step 6: Sending success callback...')
  await updateCallback({
    status: 'live',
    repoUrl: repo.html_url,
    previewUrl,
...
async function iterateExisting() {
  ...
  // Step 4: Wait for Vercel redeployment
  await updateCallback({ status: 'deploying' })

  const repoUrl = `https://github.com/${GITHUB_OWNER}/${existingRepoName}`
  let previewUrl = `https://${existingRepoName}.vercel.app`

  if (VERCEL_TOKEN) {
    console.log('Step 4: Waiting for Vercel redeployment...')
    // Brief pause for Vercel to detect the push and queue deployment
    await new Promise(resolve => setTimeout(resolve, 5000))

    const deploy = await waitForVercelDeployment(existingRepoName)

    if (!deploy.ready) {
      console.error(`Vercel redeployment failed: ${deploy.error}`)
      await updateCallback({
        status: 'failed',
        errorMessage: deploy.error || 'Vercel redeployment failed',
        repoUrl,
        repoName: existingRepoName,
        filesGenerated: files.length,
      })
      await sendTelegram([
        `*${spec.name} iteration deploy failed*`,
        '',
        `_${deploy.error}_`,
        '',
        `GitHub: ${repoUrl}`,
        `Changes are pushed — Vercel may need a manual redeploy.`,
      ].join('\n'))
      throw new Error(deploy.error || 'Vercel redeployment failed')
    }
